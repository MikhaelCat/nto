...
class TwoTowersFeatureEngineer:
    ...
    def create_two_towers_features(self, candidates_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return features_df
    def create_regression_features(self, train_df: pd.DataFrame):
        """Создание дополнительных признаков для регрессии"""
     # Взаимодействия пользователя с автором
        user_author_stats = train_df.merge(
            self.book_features[['author_id']], 
            left_on='book_id', 
            right_index=True
    ...
    # Временные паттерны
        if 'timestamp' in train_df.columns:
            ...
            user_time_patterns = train_df.groupby('user_id').agg({
            'hour': ['mean', 'std'],
            'day_of_week': lambda x: (x >= 5).mean()  # активность в выходные
            })
    
    # Популярность жанров у пользователя
